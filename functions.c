...
void loadxyfile(char *filename, int cmpfileflag)
{
   ...
   if ( !(xcmp = (float *)malloc(Nalloc*sizeof(float))) ||
        ...
        !(fcmp = (float *)malloc(Nalloc*sizeof(float))) ||
        !(dfcmp = (float *)malloc(Nalloc*sizeof(float))) )
      exit(1);
   Ncmp=0;
   while(fgets(line, SCRLEN, xyfile) != NULL) {
      ...
      if (Ncmp==Nalloc){
         ...
         fcmp  = (float *)realloc(fcmp, Nalloc *sizeof(float));
         dfcmp  = (float *)realloc(dfcmp, Nalloc *sizeof(float));
      }
...
int allocateStamps(stamp_struct *stamps, int nStamps) {                                                                     
   ...
   if (stamps) {
      for (i = 0; i < nStamps; i++) {
         ...
         if (!(stamps[i].fss = (float *)calloc(nKSStamps, sizeof(float))))
            return 1;
         
         /* **************************** */
         
         if (!(stamps[i].dfss = (float *)calloc(nKSStamps, sizeof(float))))
            return 1;
...
void buildStamps(int sXMin, int sXMax, int sYMin, int sYMax, int *niS, int *ntS,
                 ...
                 float *iRData, float *tRData, float hardX, float hardY, float fscale, float dfscale)
{
   int sPixX, sPixY;
   ...
   if (!(strncmp(forceConvolve, "i", 1)==0)) {
      ...
      if (getCenters) {
         ...
         if (nss < nKSStamps) {
            xmax = hardX ? (int)hardX : sXMin + (int)(fwStamp/2);
            ymax = hardY ? (int)hardY : sYMin + (int)(fwStamp/2);
            ...
            if (check != 0.) {
               ...
               ctStamps[*ntS].fss[nss] = fscale;
	       ctStamps[*ntS].dfss[nss] = dfscale;
               ctStamps[*ntS].nss += 1;
               if (verbose >= 2)
                  fprintf(stderr,"     #%d @ %4d,%4d,%.4e\n", nss, xmax, ymax, fscale);
            }
         }
      }
   }
      ...
      if (getCenters) {
         ...
         if (nss < nKSStamps) {
            ...
            if (check != 0.) {
               ...
               ciStamps[*niS].fss[nss] = 1. / fscale;
               ciStamps[*niS].dfss[nss] = dfscale / fscale / fscale;
               ciStamps[*niS].nss += 1;
               if (verbose >= 2)
                  fprintf(stderr,"     #%d @ %4d,%4d,%.3e\n", nss, xmax, ymax, 1./fscale);
            }
         }
      }
   }
              ...
              int xMax, int yMax, stamp_struct *stamp)
{
   ...
   for (j = yMin; j <= yMax; j++) {
      y = j - yMin;
      for (i = xMin; i <= xMax; i++) {
         refArea[(i-xMin)+y*sxLen] = data[i+j*dxLen];
...
int cutSStamp(stamp_struct *stamp, float *iData)
{
   ...
   int nss = stamp->nss;
   int sscnt = stamp->sscnt;
   int xStamp = stamp->xss[sscnt] - stamp->x0;
   int yStamp = stamp->yss[sscnt] - stamp->y0;
...
/* TS: some utility */
int message(int level, FILE* fp, const char* msg)
{
   if (verbose >= level)
