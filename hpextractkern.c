...
double make_kernel(int, int, int, int, double *);
...
int main(int argc, char **argv)
{
   ...
   if (normalize) {
      ...
      for (i = fwKernel / 2; i < ysize - fwKernel / 2; i++) {
         for (j = fwKernel / 2; j < xsize - fwKernel / 2; j++) {
            dconv[j + xsize * i] /= kSumIm;
         }
      }
   }
...
double make_kernel(int xi, int yi, int xsize, int ysize, double *kernelSol)
{
   /*****************************************************
   ...
   int i1, k, ix, iy, i;
   double ax, ay, sum_kernel, xf, yf;
   
   k = 2;
   /* range from -1 to 1 */
   xf = (xi - 0.5 * xsize) / (0.5 * xsize);
   yf = (yi - 0.5 * ysize) / (0.5 * ysize);
...
void spatial_convolve(float *image, int xSize, int ySize, 
                      float xConv, float yConv,
                      double *kernelSol, float *outim)
{
   /*****************************************************
   ...
   int kcStep = fwKernel; /* TS: for comparing to spatial_convolve in alard.c */
   ...
   for (j1 = 0; j1 < nsteps_y; j1++) {
      ...
      for(i1 = 0; i1 < nsteps_x; i1++) {
         ...
         make_kernel(x0 + i0 + hwKernel, y0 + j0 + hwKernel,
                     xSize, ySize, kernelSol);
