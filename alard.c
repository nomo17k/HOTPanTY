...
int fillStamp(stamp_struct *stamp, float *imConv, float *imRef)
{
   ...
   double ax,ay,xf,yf;
   double *im;
   float rPixX2 = 0.5 * rPixX;
   float rPixY2 = 0.5 * rPixY;
   ...
   int xi = stamp->xss[stamp->sscnt];
   int yi = stamp->yss[stamp->sscnt];
   int di = xi - hwKSStamp;
   int dj = yi - hwKSStamp;
   int nv;
...
void xy_conv_stamp(stamp_struct *stamp, float *image, int n, int ren)
{
   ...
   int xi = stamp->xss[stamp->sscnt];
   int yi = stamp->yss[stamp->sscnt];
   float fi = stamp->fss[stamp->sscnt];
   double *imc = stamp->vectors[n];
   int sub_width = fwKSStamp + fwKernel - 1;
   ...
   for (i = xi - hwKSStamp - hwKernel; i <= xi + hwKSStamp + hwKernel; i++) {
      for (j = yi - hwKSStamp; j <= yi + hwKSStamp; j++) {
         ...
         for (yc = -hwKernel; yc <= hwKernel; yc++) {
            /* TS: This is where the flux scaling needs to be done,
               just once. */
            temp[xij] += ((scaleFlux ? fi : 1.)
                          * image[i+rPixX*(j+yc)]
...
void fitKernel(stamp_struct *stamps, float *imRef, float *imConv, float *varRef, float *varConv, double *kernelSol, 
               double *meansigSubstamps, double *scatterSubstamps, int *NskippedSubstamps)
{
   ...
   check = check_again(stamps, kernelSol, imConv, imRef, varConv, varRef, meansigSubstamps, scatterSubstamps, NskippedSubstamps);
...
double check_stamps(stamp_struct *stamps, int nS, float *imRef, float *varRef, float *varConv)
{
   /*****************************************************
   ...
   if ((strncmp(forceConvolve, "b", 1)==0)) {
      ...
      for (i = 0; i < ntestStamps; i++) {
         ...
         getStampSig(&testStamps[i], testKerSol, varRef, varConv, &sig1, &sig2, &sig3);
...
void getStampSig(stamp_struct *stamp, double *kernelSol, float *varRef, float *varConv, double *sig1, double *sig2, double *sig3)
{
   int i, j, idx, index, sscnt, nsig, xRegion, yRegion, xRegion2, yRegion2;
   ...
   fss2 = stamp->fss[sscnt] * stamp->fss[sscnt];
   dfss2 = stamp->dfss[sscnt] * stamp->dfss[sscnt];
   ...
   *sig3 = 0;
   ...
   for (j = 0; j < fwKSStamp; j++) {
      ...
      for (i = 0; i < fwKSStamp; i++) {
         ...
         idx  = i+j*fwKSStamp;
         index = xRegion2+rPixX*yRegion2;
         
         tdat = temp[idx];
         idat = im[idx];
         ndat = varRef[index] + (scaleFlux ? (fss2 * varConv[index] + tdat * tdat * dfss2) : varConv[index]);
         
         diff = tdat - idat + bg;

         if ((mRData[index] & FLAG_INPUT_ISBAD) || (fabs(idat) <= ZEROVAL)) {
            continue;
         }
         else {
            temp[idx] = diff;
         ...
         if ((tdat*0.0 != 0.0) || (idat*0.0 != 0.0)) {
            mRData[index] |= (FLAG_INPUT_ISBAD | FLAG_ISNAN);
            continue;
         }
...
char check_again(stamp_struct *stamps, double *kernelSol, float *imConv, float *imRef, float *varConv, float *varRef,
                 double *meansigSubstamps, double *scatterSubstamps, int *NskippedSubstamps)
{
   /*****************************************************
   ...
   *NskippedSubstamps=0;
   ...
   for (istamp = 0; istamp < nS; istamp++) {
      ...
      if (stamps[istamp].sscnt < stamps[istamp].nss) {
         ...
         getStampSig(&stamps[istamp], kernelSol, varRef, varConv, &sig1, &sig2, &sig3);
...
void spatial_convolve2(float *image, float **variance, int xSize, int ySize, double *kernelSol, float *cRdata, int *cMask) {
   /*****************************************************
...
void spatial_convolve(float *image, float **variance, int xSize, int ySize, double *kernelSol, float *cRdata, int *cMask) {
   ...
   int       i1,j1,i2,j2,nsteps_x,nsteps_y,i,j,i0,j0,ic,jc,ik,jk,nc,ni,mbit;
   double    q, qv, kk, aks, uks;
   float     *vData=NULL;
               ...
               for (i2 = 0; i2 < kcStep; i2++) {
                  ...
                  for (jc = j - hwKernel; jc <= j + hwKernel; jc++) {
                     ...
                     for (ic = i - hwKernel; ic <= i + hwKernel; ic++) {
                        ...
                        q  += image[nc] * kk;
                        qv += ((*variance)[nc] * (convolveVariance ? kk : kk * kk));
