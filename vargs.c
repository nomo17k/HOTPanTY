...
void vargs(int argc, char *argv[]) {
   ...
   sprintf(help, "Usage:  hotpanty [options]\n");
   ...
   sprintf(help, "%s   [-ivmode]         : read noise images as inverse variance (%d)\n\n", help, ivMode);
   ...
   sprintf(help, "%s   [-sflux]          : scale point source flux in template to image (%d)\n", help, scaleFlux);
   ...
   sprintf(help, "%s   [-ocn fitsfile]   : output convolved noise image (undef)\n", help);
   ...
   for (iarg=1; iarg < argc; iarg++) {
      if (argv[iarg][0]=='-') {
         ...
         if (strcasecmp(argv[iarg]+1,"inim")==0) {
            ...
            outim = argv[++iarg];
         } else if (strcasecmp(argv[iarg]+1,"ivmode")==0) {
            ivMode = 1;
            ...
            sstampFile = argv[++iarg];
	    cmpFile = 1;
         } else if (strcasecmp(argv[iarg]+1,"sflux")==0) {
            scaleFlux = 1;
            ...
            inclConvImage = 1;
         } else if (strcasecmp(argv[iarg]+1,"ocn")==0) {
            convNoise = argv[++iarg];
   ...
   */
   ...
   /* TS: JUST TESTING to see if these two options can be used simultaneously without problems.
