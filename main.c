...
{
   long        index; /* utility variable added by TS */
   ...
   */
   ...
   FILE *rFile;
   double inv1;
   double inv2; /* TS: used to convolve sigma more efficiently */
   char        *pstr;             
   int sBorder; /*armin*/
   ...
   /* SET VERSION */
   sprintf(version, "0.1"); /* HOTPanTS version was 5.1.10 */
   ...
   fits_write_key_str(oPtr, "SOFTNAME", "HOTPanTY", "The software that differenced this image", &status);
   ...
   fits_write_key_str(oPtr, "SOFTOTHE", "Taro Sato", "Contributer", &status);
   fits_write_key_str(oPtr, "COMMENT", "", "", &status);
   ...
   if (convNoise) {
      if (!noClobber) { sprintf(scrStr, "!%s", convNoise); }
      else { sprintf(scrStr, "%s", convNoise); }
      if (fits_create_file(&ePtr, scrStr, &status) ||
          fits_create_img(ePtr, oBitpix, oNaxis, oNaxes, &status) ||
          hp_fits_copy_header(iPtr, ePtr, &status))
         printError(status);
      if (ivMode) {
         if (fits_update_key(ePtr, TSTRING, "OBJECT", "Convolved Inverse Variance Image", "", &status))
            printError(status);
      }
      else {
         if (fits_update_key(ePtr, TSTRING, "OBJECT", "Convolved Noise Image", "", &status))
            printError(status);
      }
      if (fits_close_file(ePtr, &status))
         printError(status);
   ...
   for (i = 0; i < nR; i++) {
      ...
      /* IMPORTANT NOTE:
         ...
         TS: oRData, in the original HOTPanTS, contained the variance
         estimates in the output difference image (i.e., oRData and
         eRData were summed, as commented out above), and was
         temporarily the approximate noise squared per pixes and
         eRData got reallocated later.

         However, in order to scale noise to match flux in point
         sources when the kernel is determined, oRData and eRData are
         kept separately, so at this point

         oRData = variance in input image
         eRData = variance in template image
       */
      ...
      */
         ...
         while ((status <= 2) && (flag)) {
            ...
            for (l = 0; l < nStampY; l++) {
               for (k = 0; k < nStampX; k++) {
                  ...
                  if (sstampFile) {
                     message(2, stderr, "Adding centers manually\n");
                     ...
                     for (m = 0; m < Ncmp; m++) {
                        if ((xcmp[m] > sXMin + hwKernel + 1) && (xcmp[m] < sXMax - hwKernel - 1) &&
                           ...
                           buildStamps(sXMin, sXMax, sYMin, sYMax, &niS, &ntS, 0, rXBMin, rYBMin,
                                       ...
                                       xcmp[m] - rXBMin, ycmp[m] - rYBMin, fcmp[m], dfcmp[m]);
                        }
                     }
                     if (findSSC) {
                        message(2, stderr, "Automatically finding additional centers\n");
                        /* and then choose enough stamps manually to fill the array */
                        buildStamps(sXMin, sXMax, sYMin, sYMax, &niS, &ntS, 1, rXBMin, rYBMin,          
                                    ciStamps, ctStamps, iRData, tRData, 0, 0, 0, 0);
                     }
                  }
                  else {
                     if (useFullSS) {
            ...
            if (strncmp(forceConvolve, "i", 1)==0) {
               ...
               if ((iSFrac < minFracGoodStamps) || (tSFrac < minFracGoodStamps))
                  flag = 1;
               else
                  break;
            }
         ...
         */
         ...
         if (!(strncmp(forceConvolve, "i", 1)==0)) {
            ...
            if ((strncmp(forceConvolve, "b", 1)==0)) {
               ...
               tMerit = check_stamps(ctStamps, ntS, iRData, oRData, eRData);
         ...
         if (!(strncmp(forceConvolve, "t", 1)==0)) {
            ...
            if ((strncmp(forceConvolve, "b", 1)==0)) {
               ...
               iMerit = check_stamps(ciStamps, niS, tRData, eRData, oRData);
               fprintf(stderr, "    Result : merit = %.3f\n", iMerit);
            }
            else
               iMerit = tMerit = 0;
      ...
      /* perhaps for good! */
      convTmpl = (( (strncmp(forceConvolve, "t", 1)==0) ||
      ...
      /* TS: Under normal circumstances, this convTmpl = True block is
	 the only side that gets done in hotpanty which force
	 convolution on 't'. */
      if (convTmpl) {
         ...
         if (!(kernelImIn))
            fitKernel(ctStamps, iRData, tRData, oRData, eRData, tKerSol, &meansigSubstamps, &scatterSubstamps, &NskippedSubstamps);
	 ...
	 /* TS: assume BG is subtracted and fine already
	 ...
	 */
         ...
         /*
           ...
           tRData   : Noise image (i.e., sigma in difference image)
           iRData   : Input image
           eRData   : Variance image for convolved input template (added by TS)
         ...
         */
         ...
         /* and oRData is the input noise^2 */
         if (!(kernelImIn))
            fitKernel(ciStamps, tRData, iRData, eRData, oRData, iKerSol, &meansigSubstamps, &scatterSubstamps, &NskippedSubstamps);
	 ...
	 */
         ...
         /*
           ...
           iRData   : Noise image (i.e., sigma in difference image)
           eRData   : Variance image for convolved input image (added by TS)
      ...
      */
      ...
      if (inclConvImage || convImage) {
               ...
               if (ivMode) {
                  ...
                  for (l = hwKernel; l < rPixY - hwKernel; l++) {
                     for (k = hwKernel; k < rPixX - hwKernel; k++) {
                        index = k+rPixX*l;
                        oRData[index] *= inv1;
                        eRData[index] = reciprocal( eRData[index] * inv2 ); /* TS: save inverse variance */
                     }
                  }
               }
               else {
                  for (l = hwKernel; l < rPixY - hwKernel; l++) {
                     for (k = hwKernel; k < rPixX - hwKernel; k++) {
                        index = k+rPixX*l;
                        oRData[index] *= inv1;
                        eRData[index] = sqrt(eRData[index]) * inv1; /* TS: save sigma */
                     }
                  }
               }
            }
         ...
         if (convNoise) {
            if (fits_open_file(&ePtr, convNoise, 1, &status))
               printError(status);
            if (outShort) 
               if (fits_set_bscale(ePtr, outNiBscale, outNiBzero, &status))
                  printError(status);
            if (hp_fits_write_subset(ePtr, 0, 2, oNaxes, eRData, &status,
                                     outShort, outNiBzero, outNiBscale, fpixelOutX, fpixelOutY,
                                     lpixelOutX, lpixelOutY, xBufLo, yBufLo) ||
                fits_close_file(ePtr, &status))
               printError(status);
         }
      ...
      /* TS: scale point sources before differencing */
      for (index = 0; index < Ncmp; ++index) {
         for (l = imax(ycmp[index] - hwKSStamp, 0); l < imin(ycmp[index] + hwKSStamp, rPixY); ++l) {
            for (k = imax(xcmp[index] - hwKSStamp, 0); k < imin(xcmp[index] + hwKSStamp, rPixX); ++k) {
               oRData[k + rPixX * l] *= fcmp[index];
               eRData[k + rPixX * l] *= (fcmp[index] * fcmp[index]);
            }
         }
      }
	 ...
	 for (l = hwKernel; l < rPixY - hwKernel; l++) {
      ...
      getStampStats3(convTmpl ? tRData : iRData, 0, 0, rPixX, rPixY,
                     &nsum, &nmean, &nmedian,
                     &nmode, &nsd, &nfwhm, &nlfwhm, 0x0, 0xffff, 5);
      getNoiseStats3(oRData, convTmpl ? tRData : iRData, &x2norm, &nx2norm, 0x0, 0xffff);
      fprintf(stderr, "   Mean   : %.2f\n", nmean);
      fprintf(stderr, "   Median : %.2f\n", nmedian);
      fprintf(stderr, "   Mode   : %.2f\n", nmode);
      fprintf(stderr, "   Stdev  : %.2f\n", nsd);
